"""提出物 submission_wanaider.tar.gz の組み立て + 検証。

同梱物:
    main.py               = main_wanaider.py
    wanaider_heuristic.py (ワナイダー固有ロジック)
...
使い方(~/ptcg/src で実行):
    python build_wanaider.py            # 組み立て + 検証
    python build_wanaider.py --no-test  # 組み立てのみ
"""
...
OUT = "submission_wanaider.tar.gz"
...
DECK_SRC = "../meta/top_decks/deck_wanaider_top2.csv"
POOL = "../meta/top_decks"

# (アーカイブ内の名前, コピー元)
FILES = [
    ("main.py", "main_wanaider.py"),
    ("wanaider_heuristic.py", "wanaider_heuristic.py"),
