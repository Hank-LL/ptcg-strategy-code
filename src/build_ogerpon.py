"""提出物 submission_ogerpon.tar.gz の組み立て + 検証。

同梱物:
    main.py               = main_ogerpon.py
    ogerpon_heuristic.py (キュワワー・ミル固有ロジック)
...
# cabbage patch(1049.4/42位) の構築。maco-macoo との差はクラッシュハンマー4枚と
# エネルギー転送4枚。相手のエネも自分の打点になるのでハンマーは非採用。
DECK_SRC = "../meta/top_decks/deck_ogerpon.csv"
POOL = "../meta/top_decks"

# (アーカイブ内の名前, コピー元)
FILES = [
    ("main.py", "main_ogerpon.py"),
    ("ogerpon_heuristic.py", "ogerpon_heuristic.py"),
