"""提出用エントリポイント(フーディン・ヒューリスティック版)。

alakazam_heuristic.agent に委譲するだけ。何があっても合法手を返すため、
読み込み失敗時は generic_heuristic に、それも駄目なら先頭の選択肢に落とす。
"""

try:
    import alakazam_heuristic as _ah
    _main_agent = _ah.agent
