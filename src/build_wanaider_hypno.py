"""提出物 submission_wanaider_hypno.tar.gz の組み立て + 検証。

同梱物:
    main.py               = main_wanaider.py
    wanaider_hypno_heuristic.py (さいみん装置お試し版のロジック)
...
使い方(~/ptcg/src で実行):
    python build_wanaider_hypno.py            # 組み立て + 検証
    python build_wanaider_hypno.py --no-test  # 組み立てのみ
"""
...
OUT = "submission_wanaider_hypno.tar.gz"
# 上位勢の主力構築(リプレイ160件から抽出、68試合51勝17敗=75%)。
# ロジックを上位デッキ向けに合わせた上で反復8回で比較すると、旧 deck_wanaider_top
# と互角(69.5% vs 69.0%)。ラダーでの実績(THIRD PTCG Club 105勝45敗)を採ってこちら。
DECK_SRC = "../meta/top_decks/deck_wanaider_hypno.csv"
POOL = "../meta/top_decks"

# (アーカイブ内の名前, コピー元)
FILES = [
    ("main.py", "main_wanaider_hypno.py"),
    ("wanaider_hypno_heuristic.py", "wanaider_hypno_heuristic.py"),
