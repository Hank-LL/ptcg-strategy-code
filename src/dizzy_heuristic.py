#!/usr/bin/env python3
"""【めまいの谷版】キュワワー(Comfey)ミル・デッキ専用ヒューリスティック。

バトルコロシアムを**めまいの谷**に差し替えた構築用。
めまいの谷は「こんらんが進化・退化で回復しない」ので、アノホラグサの
こんらんロックが剥がれなくなる。そのぶん**相手が進化しきる前でも**
アノホラグサに進化してよい(元版は進化で解除されるので待っていた)。
...
BATTLE_COLOSSEUM = 1264  # この構築では不採用(めまいの谷と入れ替え)
DIZZY_VALLEY = 1265      # めまいの谷: こんらんが**進化・退化で回復しない**
...
# めまいの谷が場にあるなら、相手が進化しきる前でもアノホラグサに進化してよい
USE_DIZZY_EARLY_EVOLVE = True
DIZZY_PLAY_SCORE = 5200.0
...
class Plan:
    __slots__ = ("active", "active_id", "hand_counts", "field_counts",
                 ...
                 "bench", "bench_used", "comfey_ready", "stadium_ours", "stadium_id", "dizzy_out", "op_has_ex", "no_backup", "op_locked", "op_counter_deck", "op_ability_attacker", "basic_in_hand", "op_active_ex", "op_active_can_evolve", "op_bench_ex", "boss_gain", "op_top_threat", "op_bench_damager")
...
def _build_plan(obs, state, me, op):
    ...
    try:
        ...
        p.dizzy_out = p.stadium_id == DIZZY_VALLEY
...
def _stages_left(cid):
    ...
    while stack:
        ...
        if not nxt:
            ...
            stack.extend((y, d + 1) for y in nxt)
    return best
...
def _stadium_disruption(p, cid):
    ...
    if p.stadium_id in (NEUTRAL_CENTER, BATTLE_COLOSSEUM, DIZZY_VALLEY):
...
def _bonus(o, obs, state, me, op, p, ctx=None):
    ...
    if t == OptionType.EVOLVE:
        ...
        if card is not None and card.id == BRAMBLEGHAST:
            ...
            #   ★めまいの谷が出ているなら**進化してもこんらんが解けない**ので、
            #     相手が進化しきるのを待つ必要がない(2026-08-11 ユーザー指定)。
            #     むしろ進化前のうちに縛ってしまう方が、相手は進化して
            #     解除する手も使えず動けなくなる。
            if (USE_HOLD_EVOLVE_UNTIL_FULLY_EVOLVED
                    and p.op_active_can_evolve
                    and not (USE_DIZZY_EARLY_EVOLVE and p.dizzy_out)):
    ...
    if t == OptionType.PLAY:
        ...
        # -- スタジアム: こんらんロックの本体 --
        if cid == DIZZY_VALLEY:
            # ★この構築の主役。こんらんが進化で解けなくなるので、
            #   アノホラグサのロックが刺さりっぱなしになる。
            #   自分のめまいの谷が既に出ているなら張り替えない。
            if p.dizzy_out:
                return -1500.0
            return DIZZY_PLAY_SCORE

        # -- スタジアム: ex デッキを無効化する本体(この構築では不採用) --
