...
# ★イダイナキバ(古代)。HP140・逃げ3・たね。
#   Land Collapse  コスト[無][無] ダメージ0
#     相手の山札を上から1枚トラッシュ(この番に古代のサポートを使っていたら+3枚)
#   Giant Tusk     コスト[闘][闘][無][無] 160  ← 闘エネが無いので**撃てない**
#   フラワーシャワーと違い**自分の山を減らさない**ので、山札レースで負けている
#   ときの殴り手になる。
GREAT_TUSK = 58
GREAT_TUSK_ENERGY_NEED = 2      # Land Collapse の必要エネ
POKEMON_IDS = {COMFEY, BRAMBLIN, BRAMBLEGHAST, SHAYMIN, HITMONTOP, GREAT_TUSK}
...
DISC_ENERGY_KEEP = 2           # 手札に残しておくエネの枚数
...
# --- イダイナキバ ---
# バトル場に出してよい最低エネ数(ユーザー指定は1個以上)
TUSK_PROMOTE_MIN_ENERGY = 1
TUSK_ENERGY_SCORE = 3800.0      # ベンチで育てる(キュワワーの4000より下)
TUSK_ENERGY_ACTIVE_SCORE = 5200.0   # 前に立っている牙を完成させる(最優先)
TUSK_PLACEMENT_SCORE = 5600.0   # 準備できていればキュワワー(5000)より優先
TUSK_ATTACK_SCORE = 8000.0      # フラワーシャワー(9000)より下
TUSK_PLAY_SCORE = 4400.0        # 手札からベンチへ
...
def _resolve_attacks():
    ...
    for a in all_attack():
        ...
        elif a.name == "Land Collapse":
            ATK_LAND_COLLAPSE = a.attackId
...
class Plan:
    __slots__ = ("active", "active_id", "hand_counts", "field_counts",
                 ...
                 "bench", "bench_used", "comfey_ready", "tusk_ready", "tusk_on_bench", "energy_in_hand", "stadium_ours", "stadium_id", "op_has_ex", "no_backup", "op_locked", "op_counter_deck", "op_ability_attacker", "basic_in_hand", "op_active_ex", "op_active_can_evolve", "op_bench_damager")
    ...
    def __init__(self):
        ...
        self.tusk_ready = False
        self.tusk_on_bench = False
        self.energy_in_hand = False
...
def _tusk_ready(n, p):
    """イダイナキバを前に出してよいか。n = その個体に付いているエネ数。"""
    if n >= GREAT_TUSK_ENERGY_NEED:
        return True
    return n >= 1 and p.energy_in_hand
...
def _build_plan(obs, state, me, op):
    ...
    # ★イダイナキバは**ベンチでエネを付けてから**でないとバトル場に出さない
    #   (2026-08-05 ユーザー指定)。裸で前に出すと、殴れないまま殴られるだけ。
    #   ★「準備ができている」の定義(2026-08-05 ユーザー指定):
    #     ・エネ2個が既に付いている                 → そのまま撃てる
    #     ・エネ1個 かつ **手札に付けられるエネがある** → 出したその番に2個目を足せる
    #   ランドコラプスは[無][無]=2個必要なので、1個だけで出すと殴れない番ができる。
    p.energy_in_hand = bool(p.hand_counts.get(PSYCHIC_ENERGY)
                            or p.hand_counts.get(TELEPATH_ENERGY))
    p.tusk_ready = False
    p.tusk_on_bench = False
    for b in (me.bench or []):
        if b is not None and b.id == GREAT_TUSK:
            p.tusk_on_bench = True
            if _tusk_ready(len(_energy_units(b)), p):
                p.tusk_ready = True
    try:
...
def _bonus(o, obs, state, me, op, p, ctx=None):
    ...
    if t == OptionType.ATTACK:
        _resolve_attacks()
        if o.attackId == ATK_LAND_COLLAPSE:
            # 相手の山だけを削る。自分の山を減らさないぶん、
            # 山札レースで負けているときはフラワーシャワーより価値が高い。
            return TUSK_ATTACK_SCORE
        ...
        if cid in (PSYCHIC_ENERGY, TELEPATH_ENERGY):
            if dest.id == COMFEY:
                ...
                if n == 0:
                    ...
                    return 4000.0
                return -800.0
            if dest.id == GREAT_TUSK:
                if n < GREAT_TUSK_ENERGY_NEED:
                    # ★**前に立っている**牙にあと1個で撃てるなら最優先。
                    #   ここを下げていたせいで、ベンチのキュワワー(4000)に負けて
                    #   前の牙が育たず、牙が立っている番の**52%が殴れていなかった**
                    #   (エネ0個159番 / 1個424番 / 2個528番)。
                    if o.inPlayArea == AreaType.ACTIVE:
                        return TUSK_ENERGY_ACTIVE_SCORE
                    # ベンチのうちに撃てる形まで持っていく
                    return TUSK_ENERGY_SCORE
                return -800.0
    ...
    if t == OptionType.PLAY:
        ...
        # -- ポケモン --
        if cid == GREAT_TUSK:
            return TUSK_PLAY_SCORE if p.bench_free > 0 else 100.0
        if cid == COMFEY:
...
def _placement_bonus(o, obs, state, me, p):
    ...
    if card.id == GREAT_TUSK:
        # ★エネが乗っている個体だけ前に出す。乗っていなければ絶対に出さない。
        #   準備できているならキュワワー(5000)より優先(ユーザー指定)。
        if _tusk_ready(len(_energy_units(card)), p):
            return TUSK_PLACEMENT_SCORE
        return -900.0
    if card.id == COMFEY:
        return 5000.0
    if card.id == BRAMBLIN:
